        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
        	...
        	// Main car's localization Data
          	...
          	auto previous_path_x = j[1]["previous_path_x"]; //remaining points from prev path
			...
			for(int i = 0; i < sensor_fusion.size(); i++){
				...
				vector<double> c = getFrenet(sensor_fusion[i][1],sensor_fusion[i][2], car_yaw,map_waypoints_x,map_waypoints_y);
			...
			for(int i = 0; i < 50 - previous_path_x.size(); i++){
				...
				//convertion from car to map coordinates
